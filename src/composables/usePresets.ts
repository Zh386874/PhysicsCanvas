...
export function getPreset(sceneName: string): PresetScene {
  switch (sceneName) {
    case '抛体运动':
      return presetProjectile()
    case '斜面滑块':
      return presetIncline()
    case '弹性碰撞':
      return presetCollision()
    case '磁场圆周':
      return presetMagnetic()
    case '电场偏转':
      return presetElectric()
    case '自定义':
      return customPreset()
    default:
      return presetProjectile()
