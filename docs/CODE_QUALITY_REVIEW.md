...
| 层 | 文件 | 职责 |
|----|------|------|
| 组件层 | 16 个 Vue 组件 + App.vue | UI 渲染 + 用户交互 |
| Composable 层 | 18 个 .ts 文件 | 业务逻辑封装（含本次拆分新增 6 个） |
...
当前 Vitest 358 个测试覆盖弧线碰撞、碰撞分支全覆盖、物理引擎积分、力计算、撤销重做、物体操作、预设场景、快照回放、板块定义、重置合并、弹性碰撞恢复系数、非弹性碰撞共速、摩擦力方向、板块与墙壁碰撞、完整圆法线计算、**弧线缺口角度换算**、**场区域样式**、**板块矩形模型**、**场景管理**，以及**物理定律契约**（`tests/contracts/`，覆盖自由落体匀加速、匀速运动、弹性碰撞动量/能量守恒、非弹性碰撞动量守恒）。核心物理逻辑（积分、力、碰撞）均有单元测试和集成测试覆盖。
...
| 新增：测试覆盖缺口（P1） | 已补齐：积分（physics-engine.test.ts，27 测试）、力计算（forces.test.ts，18 测试）、碰撞分支全覆盖（collision-branches.test.ts，37 测试）、弹性碰撞恢复系数（21 测试）、非弹性碰撞共速（7 测试）、摩擦力方向（11 测试）、板块碰撞（7 测试）、完整圆法线（7 测试）、弧线缺口角度换算（arc-gap-conversion.test.ts，6 测试）、场区域样式（field-region-style.test.ts，10 测试）、板块矩形模型（plate-rect-model.test.ts，5 测试）、场景管理（scene-manager.test.ts，2 测试）等。现共 358 测试，26 文件，四层覆盖。 |
